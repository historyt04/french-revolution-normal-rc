...
function completionSettingsPage(){
 ...
 ${info.completionCatalog.map(({key,label},i)=>{const r=s.rules[key],prefix='cmp'+i+'_';return `<details class="completion-rule"><summary>${E(label)}</summary><label>지급 주기<select name="${prefix}policy">${opts({first:'최초 완료만',always:'완료할 때마다',daily:'하루 최초 완료만',weekly:'주간 최초 완료만'},r.policy)}</select></label><p class="muted">${E(label)} ${E({first:'최초 완료',always:'완료',daily:'매일 첫 완료',weekly:'매주 첫 완료'}[r.policy])}</p>${rewardFields25(prefix,{card:r.card,pack:r.pack})}<label class="checkline"><input type="checkbox" name="${prefix}eventLinked" ${r.legacyEvent?'checked':''}>이벤트 기간 배수·팩 설정 연동 유지 (확률형 팩에만 적용)</label>${r.pack?.minR?'<p class="muted">기존 첫 팩 최소 등급 보장은 일반·중급·고급 확률형 팩에만 적용됩니다.</p>':''}</details>`}).join('')}
...
function saveCompletionSettings(e){e.preventDefault();const f=e.target,v=copy(info.completionSettings),val=k=>f.elements[k].value;
 for(const [i,{key}] of info.completionCatalog.entries()){const old=v.rules[key],prefix='cmp'+i+'_',reward=readReward25(f,prefix);v.rules[key]={...old,policy:val(prefix+'policy'),legacyEvent:f.elements[prefix+'eventLinked'].checked,card:reward.card,pack:reward.pack?{...reward.pack,minR:old.pack?.minR||0}:null};}
