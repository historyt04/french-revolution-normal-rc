...
let studentRosterGrade='',studentRosterClass='';
function orderedStudentRoster(){return [...(info?.students||[])].sort((a,b)=>Number(a.grade)-Number(b.grade)||Number(a.classNo)-Number(b.classNo)||Number(a.number)-Number(b.number)||String(a.name).localeCompare(String(b.name),'ko'))}
function filteredStudentRoster(){return orderedStudentRoster().filter(s=>(!studentRosterGrade||String(s.grade)===studentRosterGrade)&&(!studentRosterClass||String(s.classNo)===studentRosterClass))}
function filterStudentRoster(field,value){if(field==='grade'){studentRosterGrade=value;studentRosterClass=''}else studentRosterClass=value;render()}
function rosterStudentId(index){return filteredStudentRoster()[index]?.id}
function rosterRotate(index){const id=rosterStudentId(index);if(id)rotateStudentReady(id)}
function rosterDevices(index){const id=rosterStudentId(index);if(id)loadStudentDevicesReady(id)}
function rosterFilterMarkup(){const all=orderedStudentRoster(),grades=[...new Set(all.map(s=>Number(s.grade)))].sort((a,b)=>a-b),classes=[...new Set(all.filter(s=>!studentRosterGrade||String(s.grade)===studentRosterGrade).map(s=>Number(s.classNo)))].sort((a,b)=>a-b);return `<div class="student-roster-filters"><label>학년<select aria-label="등록 학생 학년" onchange="filterStudentRoster('grade',this.value)">${opts({'':'전체 학년',...Object.fromEntries(grades.map(n=>[n,n+'학년']))},studentRosterGrade)}</select></label><label>반<select aria-label="등록 학생 반" onchange="filterStudentRoster('class',this.value)">${opts({'':'전체 반',...Object.fromEntries(classes.map(n=>[n,n+'반']))},studentRosterClass)}</select></label></div>`}
